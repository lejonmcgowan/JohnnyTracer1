...
shared_ptr<Camera> POVParser::parseCamera(std::string block)
{
    ...
    while(input >> word)
    {
        if(word == "location")
            ...
            currentItem = &lookat;
        else if(word[word.size() - 1] == 'x' || word[word.size() - 1] == 'y' || word[word.size() - 1] == 'z'
                || word[word.size() - 1] == '>' || isdigit(word[word.size() - 1]))
        {
...
std::shared_ptr<Light> POVParser::parseLight(std::string block)
{
    ...
    //parse location
    string currentblock;
    while(currentblock.empty() || currentblock[currentblock.size() - 1] != '>')
    {
        input >> word;
        currentblock += word;
    }
...
shared_ptr<Shape> POVParser::parseBox(std::string block)
{
    ...
    currentBlock.erase(currentBlock.size() - 1);
    ...
    if(currentBlock[currentBlock.size() - 1] != '>')
        currentBlock.erase(currentBlock.size() - 1);
...
std::string POVParser::parseComments(std::string &fileString) {
    ...
    for (unsigned int i = 0; i < fileString.length(); i++)
    {
        ...
        if(!inBlockComment && !inLineComment)
        {
            ...
            if (finalString[finalString.size() - 1] == '}' || finalString[finalString.size() - 1] == '>')
            {
...
Shape &POVParser::parseObjectModifiers(Shape &shape, std::string block)
{
    ...
    while(input >> word)
    {
        ...
        if (word == "scale")
        {
            ...
            while (vecstring[vecstring.size() - 1] != '>')
            {
                input >> word;
                vecstring += word;
            }
            ...
            while(word != "}")
            {
                ...
                if(word == "ambient")
                {
                    ...
                    if(word == "rgb")
                    {
                        vec.resize(3);
                        string block;
                        while(block.empty() || block[block.size() - 1] != '>')
                        {
                            input >> word;
                            block += word;
                        }
